...
def parse_args():
    ...
    parser.add_argument('--print-only', type=bool)
    # Generate args
...
def yolo_infer2(image_path, result_path, model_path, context_norm, body_norm, ind2cat, ind2vad, write_op=False, return_op=False):
  ''' Infer on a single image - locaation specified in image_path. Bounding boxes for faces are derived using a pre-trained YOLO_v3
  :param images_path: Path to the image on which inference is to be run.
  :param result_path: Directory path to save the results (images with the predicted emotion categories and continuous emotion dimesnions).
  :param model_path: Directory path to load models and val_thresholds to perform inference.
  :param context_norm: List containing mean and std values for context images. 
  :param body_norm: List containing mean and std values for body images. 
  ...
  image_context = cv2.cvtColor(image_path, cv2.COLOR_BGR2RGB)
  try:
    bbox_yolo = get_bbox(yolo, device, image_context)
  ...
  return emo_vals, result_file_path, return_image


def yolo_infer3(image_path, result_path, model_path, context_norm, body_norm, ind2cat, ind2vad, args):
  ''' Infer on a list of images defined in images_list text file to obtain bounding boxes of persons in the images using yolo model.
  :param images_list: Text file specifying the images to conduct inference. A row in the file is Path_of_image. 
  :param result_path: Directory path to save the results (images with the predicted emotion categories and continuous emotion dimesnions).
  :param model_path: Directory path to load models and val_thresholds to perform inference.
...
if __name__=='__main__':
  ...
  if args.print_only == True:
    yolo_infer3(args.inference_file, result_path, model_path, context_norm, body_norm, ind2cat, ind2vad, args)
    exit(0)
  if args.inference_file is not None: 
    print ('inference over inference file images')
    yolo_infer2(args.inference_file, result_path, model_path, context_norm, body_norm, ind2cat, ind2vad, args)
  if args.video_file is not None:
